  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 w-full max-w-[1200px] mx-auto px-4 lg:px-6 py-10">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 flex flex-col items-start text-left">
              ...
              <p className="text-[15px] md:text-[16px] text-slate-300 max-w-lg mb-8 leading-relaxed">
                Discover the latest trends, thoughts, and best practices from our community of writers.
              </p>
            ...
            <div className="lg:col-span-5 flex justify-center lg:justify-end w-full">
              <div className="relative w-full max-w-[420px] bg-slate-950/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 md:p-8 font-mono text-[13px] md:text-[14px] leading-relaxed text-slate-300 shadow-2xl overflow-hidden select-none">
                {/* Visual code details */}
                <div className="text-slate-500 mb-2">// fetch trending posts</div>
                <div>
                  <span className="text-indigo-400 font-semibold">const</span> posts = <span className="text-indigo-400 font-semibold">await</span>
                </div>
                <div className="pl-4">
                  <span className="text-emerald-400">postService</span>.<span className="text-sky-400">findAll</span>(&#123;
                </div>
                <div className="pl-8">
                  <span className="text-amber-300">sortBy</span>: <span className="text-amber-200">'likes'</span>,
                </div>
                <div className="pl-8">
                  <span className="text-amber-300">limit</span>: <span className="text-sky-300">10</span>
                </div>
        ...
        {/* Category Filter Row */}
        <section ref={postsSectionRef} className="mb-10 border-b border-slate-200">
          <div className="flex items-center gap-4 overflow-x-auto pb-4 scrollbar-hide">
            ...
            {dbCategories.map((cat) => (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full text-[14px] font-medium transition-all duration-200 ${
                  selectedCategory?.toLowerCase() === cat.slug.toLowerCase() ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                {cat.name}
              </button>
